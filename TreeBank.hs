...
instance Parsable NonTerm where 
    parser = NonTerm `liftM` many1 upper
...
toTAGDependency (WordInfoSent wis) = TAGSentence sent depstruct
    where sent = toTagSentence (WordInfoSent wis)
          ...
          convertWI wi = (ind wi, if head == 0 then DEdge (n + 1) $ adjPos wi else DEdge head $ adjPos wi)
              where n = slength sent
                    head = adjoinInd wi
...
testData = [(
 ...
 WordInfo 23 (Word "in") (POS "IN") 20 (Spine [NonTerm "PP"]) 0 True)] 
